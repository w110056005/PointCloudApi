...
from open3d._ml3d.torch.pipelines.base_pipeline import BasePipeline
from open3d._ml3d.torch.dataloaders import get_sampler, DefaultBatcher, ConcatBatcher
from torch_dataloader import TorchDataloader
from open3d._ml3d.torch.utils import latest_torch_ckpt
...
from open3d._ml3d.utils import make_dir, PIPELINE, get_runid, code2md
from open3d._ml3d.datasets import InferenceDummySplit

log = logging.getLogger(__name__)


class SparseConvSegmentation(BasePipeline):

    def __init__(
            self,
            model,
            dataset=None,
            name='SparseConvSegmentation',
            batch_size=4,
            val_batch_size=4,
            test_batch_size=3,
    ...
    def run_test(self):
        ...
        with torch.no_grad():
            for unused_step, inputs in enumerate(test_loader):
                inputs['data']['points'] = inputs['data']['points'].to(device)
                inputs['data']['feat'] = inputs['data']['feat'].to(device)
                inputs['data']['label'] = inputs['data']['label'].to(device)
                results = model(inputs['data'])
        ...
        for epoch in range(0, cfg.max_epoch + 1):
            ...
            for step, inputs in enumerate(tqdm(train_loader, desc='training')):
                ...
                loss, gt_labels, predict_scores = model.get_loss(
                    Loss, results, inputs['data'], device)
...
PIPELINE._register_module(SparseConvSegmentation, "torch")